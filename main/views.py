    ...
    if request.method == "GET":
        ...
        if is_admin_logged_in:
            ...
            if user_type == "reviewer":
                ...
                reviewer.save()
                if is_accepted:
                    message = "You have been deauthorised in Conference System as a reviewer."
                else:
                    message = "You have been authorised in Conference System as a reviewer."
            elif user_type == "chair":
                chair = models.Chair.objects.get(email=user_mail)
                chair.is_accepted = (not is_accepted)
                chair.save()
                if is_accepted:
                    message = "You have been deauthorised in Conference System as a chair."
                else:
                    message = "You have been authorised in Conference System as a chair."
            email_send_thread = Thread(target=Utils.send_email, args=("Your status in conference system.",
                                                                                       message,
                                                                                       user_mail,
                                                                                       email_lock
                                                                                       ))
